...
def template_whois():
    return f'''Who is {adjective_positive()}?
    Yes! You are!'''

def template_practical():
    return f'''Don't {verb()} {noun_plural()}!
    They're just not {adjective()} enough.'''

def template_listen():
    return f'''Listen to the {people_plural()}.
    They know how to {verb()} {noun_plural()}.'''

def template_madness():
    return f'''Be careful around {random.choice([noun_plural(), people_plural()])}.
    {capitalize_first_letter(sometimes())}, they {verb_intransitive()}.
    It's {quantifier()}.'''
    
def template_riddle():
	temp = noun_singular()
	return f'''What is {adjective()}, 
	{adjective()},
	and {verb_third_person()} {noun_plural()}?
	
	{capitalize_first_letter(a_or_an(temp))} {temp}'''
	
def template_sphinx():
	return f'''I am {adjective()},
	I {verb_intransitive()} {adverb()},
	and I {verb()} {random.choice([noun_plural(), people_plural()])}.
...
def template_practical():
    return f'''Don't {verb()} {noun_plural()}!
    It's just not {adjective()} enough.'''
...
def template_madness():
    return f'''Be careful around {random.choice([noun_plural(), people_plural()])}.
    Sometimes, they {verb_intransitive()}.
    It's {quantifier()}.'''
